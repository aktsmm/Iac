...
// ロードバランサー用のパブリックIPアドレスを作成
resource loadBalancerPip 'Microsoft.Network/publicIPAddresses@2021-03-01' = {
  name: 'WSFC-LB-PIP'
  location: location
  sku: {
    name: 'Standard'
    ...
    frontendIPConfigurations: [
      {
        ...
        properties: {
          publicIPAddress: {
            id: loadBalancerPip.id  // パブリックIPアドレスの参照
          }
        }
      }
    ]
    ...
    loadBalancingRules: [
      {
        ...
        properties: {
          frontendIPConfiguration: {
            id: resourceId('Microsoft.Network/loadBalancers/frontendIPConfigurations', 'WSFC-LB', 'LoadBalancerFrontend')  // フロントエンドIP構成の参照
          }
          backendAddressPool: {
            id: resourceId('Microsoft.Network/loadBalancers/backendAddressPools', 'WSFC-LB', 'WSFCBackendPool')  // バックエンドプールの参照
          }
          probe: {
            id: resourceId('Microsoft.Network/loadBalancers/probes', 'WSFC-LB', 'WSFC-HealthProbe')  // ヘルスプローブの参照
          }
          protocol: 'Tcp'
          frontendPort: 3380  // WSFCで使用するポート
          backendPort: 3380
